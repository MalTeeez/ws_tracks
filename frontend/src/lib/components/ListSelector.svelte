<script lang="ts">
	...
	let display_value_list = value_list.slice();
	for (const value of display_value_list) {
		max_chars = Math.max(max_chars, value.length)
	}
	for (let i = 0; i < display_value_list.length; i++) {
		const value = display_value_list[i];
		if (value) {
			display_value_list[i] = display_value_list[i] + "&nbsp;".repeat(max_chars-value.length);
		}
	}
	let selected_display_value: string | undefined = $state(display_value_list.at(0));

	$effect(() => {
		if (selected_display_value) {
			selected_value = selected_display_value.replaceAll("&nbsp;", "")
		}
	})
	...
	<div class="pointer-events-auto">
		{#if expanded}
			...
			>
				...
				<div tabindex="-1" class="menu-container" role="menu" style="box-shadow: rgb(1, 4, 9) 0px 0px 16px;">
					{#each display_value_list as value}
						<button
							class="menu-item font-mono font-semibold text-neutral-300 antialiased hover:bg-neutral-500"
							title=" Option to select {value} as the current value"
							tabindex="-1"
							onclick={() => {
							...
							><div class="text-wrap-flow">
								<span class="text-wrap truncate">{@html value}</span>
							</div></button
						>
					{/each}
				</div>
